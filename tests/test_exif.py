...
# ── ImageExifReader — TIFF late-IFD fast-path ────────────────────────────

class TestImageExifReaderTiffFastPath(unittest.TestCase):
    """Tests for the scanner-TIFF early-exit: when IFD0 is past 50 % of a
    large file, get_raw() must return {} without invoking Pillow at all."""

    def _write_tiff_stub(
        self,
        ifd0_offset: int,
        file_size: int,
        little_endian: bool = True,
    ) -> str:
        """Write a minimal TIFF stub: valid 8-byte header + zero-fill to file_size."""
        endian = "<" if little_endian else ">"
        bo = b"II" if little_endian else b"MM"
        header = bo + struct.pack(endian + "HI", 42, ifd0_offset)
        with tempfile.NamedTemporaryFile(suffix=".tif", delete=False) as f:
            f.write(header)
            remaining = file_size - len(header)
            chunk = b"\x00" * min(4096, remaining)
            while remaining > 0:
    ...
    def _mock_pil_open(self):
        """Return a (mock_image_mod, sys_modules_patch) pair.

        ``from PIL import Image`` inside get_raw() resolves Image via the PIL
        mock's .Image attribute, so mock_pil.Image must equal mock_image_mod.
        """
        mock_ctx = MagicMock()
        mock_ctx.__enter__ = MagicMock(return_value=mock_ctx)
        mock_ctx.__exit__ = MagicMock(return_value=False)
        mock_ctx.getexif.return_value.__bool__ = MagicMock(return_value=False)

        mock_image_mod = MagicMock()
        mock_image_mod.open.return_value = mock_ctx

        mock_pil = MagicMock()
        mock_pil.Image = mock_image_mod   # PIL.Image resolves to mock_image_mod
        ...
        return mock_image_mod, patch_dict

    def test_late_ifd_large_file_skips_pillow(self):
        """IFD0 at 99 % of a 600 KB file → fast-path returns {} without Pillow."""
        file_size = 600 * 1024
        fname = self._write_tiff_stub(int(file_size * 0.99), file_size)
        self.addCleanup(os.unlink, fname)

        mock_image_mod = MagicMock()
        mock_image_mod.open.side_effect = AssertionError(
            "Pillow must not be called for end-of-file IFD TIFF"
        )
        with patch.dict("sys.modules", {
            "PIL": MagicMock(), "PIL.Image": mock_image_mod,
            "PIL.ExifTags": MagicMock(),
        }):
            result = ImageExifReader().get_raw(Path(fname), file_size=file_size)

        self.assertEqual(result, {})
        mock_image_mod.open.assert_not_called()

    def test_big_endian_late_ifd_skips_pillow(self):
        """Big-endian TIFF (Motorola byte order) with late IFD0 is also fast-pathed."""
        file_size = 600 * 1024
        fname = self._write_tiff_stub(int(file_size * 0.99), file_size, little_endian=False)
        self.addCleanup(os.unlink, fname)

        mock_image_mod = MagicMock()
        mock_image_mod.open.side_effect = AssertionError("must not call Pillow")
        with patch.dict("sys.modules", {
            "PIL": MagicMock(), "PIL.Image": mock_image_mod,
            "PIL.ExifTags": MagicMock(),
        }):
    ...
    def test_no_file_size_skips_fast_path(self):
        """Callers that don't supply file_size never trigger the fast-path."""
        file_size = 600 * 1024
        fname = self._write_tiff_stub(int(file_size * 0.99), file_size)
        self.addCleanup(os.unlink, fname)

        mock_image_mod, pil_mods = self._mock_pil_open()
        with patch.dict("sys.modules", pil_mods):
            ImageExifReader().get_raw(Path(fname))  # no file_size → proceeds to Pillow

        mock_image_mod.open.assert_called_once()

    def test_early_ifd_proceeds_to_pillow(self):
        """IFD0 at offset 8 (normal TIFF) is never fast-pathed."""
        file_size = 600 * 1024
        fname = self._write_tiff_stub(8, file_size)
        self.addCleanup(os.unlink, fname)

        mock_image_mod, pil_mods = self._mock_pil_open()
        with patch.dict("sys.modules", pil_mods):
            ImageExifReader().get_raw(Path(fname), file_size=file_size)

        mock_image_mod.open.assert_called_once()

    def test_small_file_with_late_ifd_proceeds_to_pillow(self):
        """Files under 512 KB are never fast-pathed regardless of IFD0 position."""
        file_size = 400 * 1024   # below the 512 KB minimum
        fname = self._write_tiff_stub(int(file_size * 0.99), file_size)
        self.addCleanup(os.unlink, fname)
    ...
    def test_non_tiff_not_affected(self):
        """A JPEG (no TIFF magic bytes) is never subject to the fast-path check."""
        with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as f:
            f.write(b"\xff\xd8\xff\xe0" + b"\x00" * (600 * 1024))
            fname = f.name
        self.addCleanup(os.unlink, fname)

        mock_image_mod, pil_mods = self._mock_pil_open()
        with patch.dict("sys.modules", pil_mods):
            ImageExifReader().get_raw(Path(fname), file_size=600 * 1024)

        mock_image_mod.open.assert_called_once()

    def test_tiff_ifd0_offset_little_endian(self):
        with tempfile.NamedTemporaryFile(suffix=".tif", delete=False) as f:
            f.write(b"II" + struct.pack("<HI", 42, 99999))
            fname = f.name
        self.addCleanup(os.unlink, fname)
        self.assertEqual(ImageExifReader._tiff_ifd0_offset(Path(fname)), 99999)

    def test_tiff_ifd0_offset_big_endian(self):
        with tempfile.NamedTemporaryFile(suffix=".tif", delete=False) as f:
            f.write(b"MM" + struct.pack(">HI", 42, 99999))
            fname = f.name
        self.addCleanup(os.unlink, fname)
        self.assertEqual(ImageExifReader._tiff_ifd0_offset(Path(fname)), 99999)

    def test_tiff_ifd0_offset_non_tiff_returns_none(self):
        with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as f:
            f.write(b"\xff\xd8\xff\xe0\x00\x10JFIF")
            fname = f.name
        self.addCleanup(os.unlink, fname)
        self.assertIsNone(ImageExifReader._tiff_ifd0_offset(Path(fname)))

    def test_tiff_ifd0_offset_missing_file_returns_none(self):
        self.assertIsNone(ImageExifReader._tiff_ifd0_offset(Path("/nonexistent/file.tif")))
